...
const ManageIndustry = ({ industryData, startupOptions }: ManageIndustryProps) => {
  const [name, setName] = useState(industryData?.name || '');
  const [slug, setSlug] = useState(industryData?.slug || '');
  const [selectedStartups, setSelectedStartups] = useState<string[]>(industryData?.startupIds.map(id => id.toString()) || []);
    ...
    <Fragment>
      <div className="p-4">
        ...
        <div className="mb-4">
          ...
          <div className="flex">
            <input
              ...
              readOnly
            />
            <button type="button" className="ti-btn ti-btn-light" onClick={() => setSlug(name.toLowerCase().replace(/\s+/g, '-'))}>
              Refresh
            </button>
          </div>
        </div>
        <div className="mb-4">
          ...
          <select className="form-select" onChange={handleStartupChange} defaultValue="">
          ...
          <div className="mt-2">
            {selectedStartups.map((startup, idx) => (
              <div key={idx} className="flex items-center justify-between p-2 bg-white/10 rounded mb-2">
                <span>{startup}</span>
                <button type="button" className="text-red-500" onClick={() => removeStartup(startup)}>
                  &times;
                </button>
              </div>
            ))}
